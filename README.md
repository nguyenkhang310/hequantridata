...
> **Lưu ý thực hành:** Các kịch bản lỗi đã được cấu hình tự động mô phỏng luồng chạy (sử dụng Transaction và biến Session). Bạn chỉ cần mở từng file bằng phần mềm MySQL Workbench/Navicat và **ấn chạy toàn bộ (Execute All)**. Hệ thống sẽ tự động in ra các bảng kết quả chứng minh lỗi và cách khắc phục.
...
- **Cách thực hiện:** Chạy file `sql/demo/Loi2_DirtyRead.sql`.
...
- **Cách thực hiện:** Chạy file `sql/demo/Loi3_NonRepeatableRead.sql`.
...
- **Cách thực hiện:** Chạy file `sql/demo/Loi4_PhantomRead.sql`.
