...
> **Lưu ý thực hành sau khi cập nhật:** `Loi1_LostUpdate.sql` có thể chạy **Execute All** trong một tab. Các lỗi `Dirty Read`, `Non-Repeatable Read`, `Phantom Read` và `Deadlock` cần **2 cửa sổ kết nối MySQL riêng biệt** để chứng minh đúng bản chất nhiều transaction. Không dùng 2 tab chung một connection trong MySQL Workbench, vì chúng có thể dùng chung `Connection_ID` và làm mất hiệu ứng khóa/transaction.
...
- **Nơi chụp ảnh:** Chụp lại các bảng kết quả hiển thị ở khung Output (như hình bạn đã chụp phần `Mong_Duoc_32_Nhung_Chi_Co` và `Ket_Qua_Dung_La_32`). Bạn có thể chụp chung 2 tab kết quả lại. Lưu ảnh thành `docs/lost_update.png` và cập nhật ảnh vào thư mục `docs/`.
...
- **Cách thực hiện:** Mở `sql/demo/Loi2_DirtyRead.sql` ở 2 cửa sổ kết nối riêng. Chạy lần lượt các khối `SESSION A` và `SESSION B` theo số bước trong file. Kết quả lỗi: Session B đọc thấy điểm `9.00` dù Session A chưa commit. Kết quả fix: với `READ COMMITTED`, Session B chỉ thấy điểm thật `7.50`.
- **Nơi chụp ảnh:** Chụp lại bảng kết quả thấy rõ cột `Du_Lieu_Ban_Doc_Duoc` (ví dụ 9.0) và bảng `Diem_Thuc_Te_Sau_Rollback` (ví dụ 7.5). Lưu ảnh thành `docs/dirty_read.png`.
...
- **Cách thực hiện:** Mở `sql/demo/Loi3_NonRepeatableRead.sql` ở 2 cửa sổ kết nối riêng. Chạy theo thứ tự bước trong file. Kết quả lỗi: trong `READ COMMITTED`, lần đọc 1 thấy `7.50`, sau khi Session B commit thì lần đọc 2 thấy `9.00`. Kết quả fix: trong `REPEATABLE READ`, cả 2 lần đọc trong Session A vẫn giữ `7.50`.
- **Nơi chụp ảnh:** Chụp lại bảng kết quả cho thấy giá trị ở "Lần đọc 1" khác với giá trị ở "Lần đọc 2". Lưu ảnh thành `docs/non_repeatable_read.png`.
...
- **Cách thực hiện:** Mở `sql/demo/Loi4_PhantomRead.sql` ở 2 cửa sổ kết nối riêng. Chạy theo thứ tự bước trong file. Kết quả lỗi: trong `READ COMMITTED`, lần đếm 1 là `30`, Session B chèn thêm 1 dòng, lần đếm 2 thành `31`. Kết quả fix: trong `REPEATABLE READ`, Session A vẫn thấy `30` trong cùng transaction.
- **Nơi chụp ảnh:** Chụp lại sự chênh lệch số lượng bản ghi ở 2 bảng kết quả truy vấn. Lưu ảnh thành `docs/phantom_read.png`.
